...
---@param method (string) The invoked LSP method
---@param params (table): Parameters for the invoked LSP method
---@return nil, vim.lsp.protocol.ErrorCodes.MethodNotFound
...
function M.merge_dispatchers(dispatchers)
  if dispatchers then
...
---@return (string) message containing encoded message and `Content-Length` attribute
...
---@return PipeClient
function M.new_client(dispatchers, transport)
  local state = {
    message_index = 0,
    message_callbacks = {},
...
function M.public_client(client)
  local result = {}
...
local function request_parser_loop()
  ...
  while true do
    ...
    if start then
      ...
      local rest = ""
      if body_length > content_length then rest = last_chunk:sub(content_length - body_length) end
